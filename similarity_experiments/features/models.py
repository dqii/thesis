...
class GCNNModelMedium(object):
    def __init__(self, x, y, settings, num_chan, num_features, lr, reg, dropout, training):
        ...
        self.weights = {
            ...
            'wd1': tf.Variable(initializer([4*4*256, 1024])),
        ...
        self.biases = {
            ...
            'bc2': tf.Variable(initializer([128])),
            'bc3': tf.Variable(initializer([256])),
            'bc4': tf.Variable(initializer([256])),
...
class GCNNModelLarge(object):
    def __init__(self, x, y, settings, num_chan, num_features, lr, reg, dropout, training):
        ...
        self.weights = {
            ...
            'wc3': tf.Variable(initializer(w_shape3)),
            'wc4': tf.Variable(initializer(w_shape4)),
            'wc5': tf.Variable(initializer(w_shape5)),
            'wd1': tf.Variable(initializer([4*4*128, 512])),
        ...
        self.biases = {
            'bc1': tf.Variable(initializer([128])),
            'bc2': tf.Variable(initializer([128])),
            'bc3': tf.Variable(initializer([128])),
            'bc4': tf.Variable(initializer([128])),
            'bc5': tf.Variable(initializer([128])),
            'bd1': tf.Variable(initializer([512])),
    ...
    def feature_model(self):
        conv1 = gcnn2d('conv1', self.x, self.weights['wc1'], self.biases['bc1'], 1, self.training, self.dropout,
                       self.gconv_indices['1'], self.gconv_shape_info['1'])
        conv2 = gcnn2d('conv2', conv1, self.weights['wc2'], self.biases['bc2'], 1, self.training, self.dropout,
        ...
        conv5 = gcnn2d('conv5', conv4, self.weights['wc5'], self.biases['bc5'], 2, self.training, self.dropout,
